...
public class Protocol_G6 {
	...
	public Protocol_G6(boolean _machine, int _port, byte _system_id){
		machine = _machine;
		port = _port;
	...
	public void execute() {
		...
		Log.d("Protocol_G6", "Protocol G5 is executing...");
		...
		try {
			...
			while(true){
				try {
					...
					Log.d("Protocol_G6", "A ler...");
					rqst request = (rqst)appIn.readObject();
					
					Log.d("Protocol_G6", "Pacote recebido");
					byte id = request.id;
