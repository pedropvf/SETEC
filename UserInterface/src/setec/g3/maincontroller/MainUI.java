...
public class MainUI extends Activity implements SensorEventListener{
		...
		public void run() {
			...
			if (targetMode){
					...
					if((volta1(targetDegree) >= -130) && (volta1(targetDegree) <= -10) ){
						...
						speak("Volte para tr�s" + Integer.toString((int) coordDist(location.getLatitude(),location.getLongitude(),Olat,Olong)) + "metros.");
						left=false;
						front=false;
						back=false;
						right=false;
	...
	public void lineOfFireUpdateRequest(){
		...
		if(language==UILanguage.EN){
			...
			builder.setTitle("Actualiza��o da linha de fogo");
...
/*                      System.out.println("teste requestDG:");
    ...
    public void notifyOutOfAlert(){
    	...
    	if(language==UILanguage.EN){
	    	...
	    	toastMessage("Notificação de saída de alarme enviada",Toast.LENGTH_SHORT, 0, 0);
    	}
    }
}
