...
public class Protocol_G6 {
    ...
    long t_espera = 15000;
    // N¬∫ retransmissoes definidas
    int numTransm = 3;
    // caso Backend: array que guarda os Socket IDs dos n√≥s com GSM
    ...
    public Protocol_G6(boolean machine, int port, byte system_id) {
        ...
        // Cria um novo n√≥ e adiciona a rota para si mesmo
    ...
    public void execute() throws IOException, ClassNotFoundException, InterruptedException {
        ...
        // Thread respons√°vel por estar √† escuta dos requests do SocketApp          
        ReadAppSocket AndroidRequests = new ReadAppSocket(Protocol_G6.this);
        new Thread(AndroidRequests).start();

        // Thread respons√°vel por analisar as filas onde s√£o postos os 
                        ...
                        switch (request.id) {
                            ...
                            case (byte) 0x33:
                                ...
                                if (request.spec == 0x00) {
                                    ...
                                    if (false) {
                                        Log.d("Protocol_G6","N√≥ " + this.threadProtocol.deviceID + " ganhou GSM");
                                    }
                                } else if (request.spec == 0x11) {
                                    threadProtocol.node.GSM = false;
                                    rotas.removeEntradaTabela(threadProtocol.node.tabRota, CENTRAL);
                                    if (DEBUG) {
                                        Log.d("Protocol_G6","N√≥ " + this.threadProtocol.deviceID + " perdeu GSM");
                                    }
                                }
                                response = new rspns((byte) 0x00);
                            ...
                            default:
                                if (DEBUG) {
                                    Log.d("Protocol_G6","Request Android Inv√°lido " + request.id + " " + request.spec);
                                }
                        }
                        ...
                        switch (request.id) {
                            case 0x00:
                               ...
                               /*if(threadProtocol.tabelaSocketID.containsValue(request.spec) == false){
                                ...
                                //adiciona socketID √† fila com os sockets todos usados
                            ...
                            case 0x55:
                                ...
                                //Log.d("Protocol_G6","N√≥ 1 recebeu mensagem do Backend para enviar para o n√≥ " + ((int) request.spec)
                                //+ ": " + pacote.binaryStringToText(pacote.byteArray2binaryString(request.packet)));
                                ...
                                break;
                            default:
                                Log.d("Protocol_G6","Request Backend Inv√°lido" + request.id + " " + request.spec);
                                break;
                        }
                    } catch (IOException | ClassNotFoundException ex) {
                        Logger.getLogger(Protocol_G6.class.getName()).log(Level.SEVERE, null, ex);
                    ...
                    synchronized (threadProtocol.node.fila_espera_ACK_RRply) {
                        ...
                        if (threadProtocol.node.filaout.size() != 0 || threadProtocol.node.fila_espera_ACK_RRply.size() != 0
                            ...
                            try {
                                ...
                                if (filaEspera.getDest(threadProtocol.node.filaout) == CENTRAL && threadProtocol.node.GSM == true) {
                                    ...
                                    if (response.id != (byte) 0xFF) {
                                        if ((pacote.getTypePacote(filaEspera.verElementoCabeÁaFila(threadProtocol.node.filaout).dados) == 0 && pacote.getSourcePacote(filaEspera.verElementoCabeÁaFila(threadProtocol.node.filaout).dados) == threadProtocol.node.nodeIdentification)
                                            ...
                                            //Se Ack ou RRply n√£o chegarem dentro de um x de tempo retransmite
                                ...
                                //Destino √© central e n√≥ n√£o tem GSM ou dest!=central
                                ...
                                for (int i = 0; i < threadProtocol.node.fila_espera_ACK_RRply.size(); i++) {
                                    ...
                                    // Verifica se Ntrnsmissoes √© maior do que o definido
                    ...
                    if (threadProtocol.node.filaout.size() != 0) {
                        ...
                        try {
                            ...
                            /*Log.d("Protocol_G6","O meu destino √© "+filaEspera.verElementoCabeÁaFila(
                                ...
                                // Primeiro tenta enviar directamente para o n√≥ de destino (pode ser que esse n√≥ tenha ganho GSM entretanto
                            /*for (int i = 0; i < 256; i++) {
                                ...
                                if (spec != (byte) NO_SOCKET) {
                                    if (pacote.getTypePacote(filaEspera.verElementoCabeÁaFila(
                                        ...
                                        } else { // MODIFICA√á√ÉO: S√≥ remove se n√£o receber 0xEE.
                                    ...
                                    //Log.d("Protocol_G6","N√£o existem sockets dispon√≠veis na central");
                                }
