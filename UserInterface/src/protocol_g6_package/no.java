...
import static protocol_g6_package.rotas.ENDERE荗_NULO;
...
public class no {
    ...
    int Flag_app_Back; //se =0 茅 App Se =1 茅 BackEnd
    boolean GSM;
    int espera_rrply; //Quando est谩 脿 espera de um rrply n茫o envia mais nenhum
    ...
    public no(Queue<filaEspera> queue, Queue<filaEspera> queue2, Queue<filaEspera> queue3, List<waitingPackets> queue4, Queue<filaEspera> queue5, Queue<rqst> queue6, rotas[] table1, tabelaValidade[] table2, int nodeID, Queue<Byte> sckQueue) {
        ...
        GSM = false; // Inicialmente 茅 falso
        espera_rrply = 0;
    ...
    public void reencaminhaPacote(String packet, int dest, int nHop) {
        ...
        /*Log.d("no","\n\nFILA OUT");
         ...
         filaEspera.imprimirCabe鏰FilaEspera(this.filaout);
         Log.d("no","\n\n");
    ...
    public int recebePacote() {
        ...
        if (filaEspera.verElementoCabe鏰Fila(this.filain) != null) { //verifica se tem elementos na fila

            /*
             if (nodeIdentification==3){
             Log.d("no","\nXXXXXX");
             pacote.imprimePacote(filaEspera.verElementoCabe鏰Fila(this.filain).dados);
             }*/
            /*if(this.nodeIdentification==1){  
            ...
            if (versao != VERSAO_PROTOCOLO) { //se n for o nosso protocolo descarta
                if (DEBUG_FINAL) {
                    if (this.nodeIdentification != 1) {
                        Log.d("no","Pacote recebido pelo n贸 " + this.nodeIdentification + " n茫o corresponde 脿 nossa vers茫o");
                    } else {
                        Log.d("no","Pacote recebido pelo Backend n茫o corresponde 脿 nossa vers茫o");
                    }
                }
            ...
            if (pacote.verificaErros(packet) == -1) { //se tiver erros descarta
                if (DEBUG_FINAL) {
                    if (this.nodeIdentification != 1) {
                        Log.d("no","Pacote recebido pelo n贸 " + this.nodeIdentification + " cont茅m erros");
                    } else {
                        Log.d("no","Pacote recebido pelo Backend cont茅m erros");
                    }
            ...
            if ((tabelaValidade.verificaValidade(this.tabValidade, id, source) == 0
                ...
                if (DEBUG_FINAL) {
                    if (this.nodeIdentification != 1) {
                        Log.d("no","Pacote recebido pelo n贸 " + this.nodeIdentification
                                + " 茅 um pacote repetido do tipo " + pacote.getTypePacote(packet)
                                + " proveniente do n贸 " + source + " e enviada pelo n贸 " + pacote.getOrigSourcePacote(packet));
                    } else {
                        Log.d("no","Pacote recebido pelo Backend 茅 um pacote repetido do tipo " + pacote.getTypePacote(packet)
                                + " proveniente do n贸 " + source + " e enviada pelo n贸 " + pacote.getOrigSourcePacote(packet));
                    }
            ...
            if (pacote.getNextHopPacote(packet) != this.nodeIdentification
                ...
                if (DEBUG_FINAL) {
                    if (this.nodeIdentification != 1) {
                        Log.d("no","Pacote recebido pelo n贸 " + this.nodeIdentification + " n茫o era para si destinado");
                    } else {
                        Log.d("no","Pacote recebido pelo Backend n茫o era para si destinado");
                    }
            ...
            if ((pacote.getNextHopPacote(packet) == this.nodeIdentification)
                ...
                if (type == 0 || type == 3) { //se for info ou ack reencaminha
                    if (nHop != -1) {
                        ...
                        if (DEBUG) {
                            Log.d("no","Pacote recebido pelo n贸 " + this.nodeIdentification + " 茅 reencaminhado (O n贸 茅 apenas o next hop do pacote)");
                        }
                    } else {
                        ...
                        if (DEBUG) {
                            Log.d("no","Pacote recebido pelo n贸 " + this.nodeIdentification + " era para ser reencaminhado mas devido 脿 inexist锚ncia de rota cria-se RRequest ");
                        }
                        this.rreq = false;
                    }
                    ...
                    if (nHop == -1) {
                        //Log.d("no","Pacote de RReply recebido pelo n贸 " + this.nodeIdentification + ", no entanto n existe rota para retransmitir!!");
                    } else {
                        reencaminhaPacote(packet, dest, nHop);
                        if (DEBUG_DETAILED) {
                            Log.d("no","Pacote de RReply recebido pelo n贸 " + this.nodeIdentification + " 茅 reencaminhado (n贸 n茫o 茅 o destino do pacote)");
                        }
                    }
                }
                ...
                if (type == 0) { //se for BROADCAST reencaminha
                    ...
                    if (DEBUG) {
                        Log.d("no","Pacote de info (broadcast) recebido pelo n贸 " + this.nodeIdentification + " 茅 retransmitido em broadcast");
                    }
                    ...
                    if (DEBUG_DETAILED) {
                        Log.d("no","Pacote de RRequest recebido pelo n贸 " + this.nodeIdentification + " 茅 enviado em broadcast (n贸 n茫o 茅 o destino do pacote)");
                    }
                }
                ...
                if (type == 0) { //se for o destino juntar os pacotes e mandar dados p android
                    ...
                    if (pacote.getFragmentFlagPacote(packet) == 1) {
                        ...
                        if (DEBUG_FINAL) {
                            if (this.nodeIdentification != 1) {
                                Log.d("no","N贸 " + this.nodeIdentification + " recebe Info (com fragmenta缑)");
                            } else {
                                Log.d("no","Backend recebe Info (com fragmenta缑)");
                            }
                       ...
                       // Log.d("no","Je " + this.nodeIdentification +" Recebi pacote de info do no "+source);
                        ...
                        if (DEBUG_FINAL) {
                            if (this.nodeIdentification != 1) {
                                Log.d("no","N贸 " + this.nodeIdentification + " recebe Info (sem fragmenta缑)");
                            } else {
                                Log.d("no","Backend recebe Info (sem fragmenta缑)");
                            }
                    ...
                    if (DEBUG_FINAL) {
                        if (this.nodeIdentification != 1) {
                            Log.d("no","N贸 " + this.nodeIdentification + " recebe RRequest");
                        } else {
                    ...
                    if (DEBUG_FINAL) {
                        if (this.nodeIdentification != 1) {
                            Log.d("no","N贸 " + this.nodeIdentification + " recebe RReply");
                        } else {
                    ...
                    for (int i = 0; i < this.fila_espera_ACK_RRply.size(); i++) {
                        ...
                        if (pacote.getTypePacote(compPacket) == 1 && pacote.getDestinoPacote(compPacket) == source) {
                            ...
                            if (DEBUG) {
                                Log.d("no","Pacote RReply recebido pelo n贸 " + this.nodeIdentification + ". Actualiza tabelas");
                            }
                    ...
                    //Log.d("no","Pacote Ack recebido pelo n贸 "+this.nodeIdentification);
                    //pacote.imprimePacote(packet);
                     //Log.d("no","Eu "+this.nodeIdentification+ " Recebeu ack do no "+ source);
                    if (DEBUG_FINAL) {
                        if (this.nodeIdentification != 1) {
                            Log.d("no","N贸 " + this.nodeIdentification + " recebe Ack");
                        } else {
    ...
    public int preparaPacote() {
        ...
        if (filaEspera.verElementoCabe鏰Fila(this.fila_dados_in) != null) {
                ...
                if (this.Flag_app_Back == 0) {
                    ...
                    if (filaEspera.verElementoCabe鏰Fila(this.fila_dados_in) != null) {
                        ...
                        if (rotas.getEntradaTabela(this.tabRota, dest) != -1) {
                            ...
                            if (DEBUG_DETAILED) {
                                Log.d("no","N贸 " + this.nodeIdentification + " quer enviar informa缑 e tem rota.");
                            }
                            int nHop = rotas.getEntradaTabela(this.tabRota, dest);
                            ...
                            if (DEBUG_DETAILED) {
                                Log.d("no","N贸 " + this.nodeIdentification + " quer enviar informa缑 mas n茫o tem rota. Envia RRequest e espera RReply ");
                            }
                            int nHop = 0;
                            int TotalFragments = this.calculaNumFragmentos(dados);
                            int FragmentFlag;
                    ...
                    if (filaEspera.verElementoCabe鏰Fila(this.fila_dados_in) != null) {
                        ...
                        // MODIFICA脟脙O: Para a central p玫e-se directamente na filaout a central n茫o tem tabela de rotas
                        int nHop = 0;
                        int TotalFragments = this.calculaNumFragmentos(dados);
                        int FragmentFlag;
                        if (TotalFragments == 0) {
    ...
    //Calcula N潞 Fragmentos para uns determinados dados
